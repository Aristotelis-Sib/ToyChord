from flask import Flask, redirect, url_for, request, logging, abort, render_template,Blueprint
import os
import requests
...
def get(key):
    ...
    if key=="*":
        ...
        if succ==myglobal.node.host:
            return "Node (ip:port):"+ myglobal.node.host+"|"+"Data stored :"+ str(list(myglobal.node.dict_db.values()))+"\n",200
        myglobal.node.dict_qid[str(qid)]=(event,"Node (ip:port):"+ myglobal.node.host+"|"+"Data stored :"+ str(list(myglobal.node.dict_db.values()))+"\n" )        
    ...
    if DHTNode.between_right_inclusive(h, myglobal.node.get_hash(myglobal.node.get_predecessor()), myglobal.node.id) and val=="":    
...
def qresponse():
    ...
    if from_node==None:
        ...
        if from_node==myglobal.node.get_predecessor():
            ...
            myglobal.node.dict_qid[str(qid)]=(myglobal.node.dict_qid[str(qid)][0],myglobal.node.dict_qid[str(qid)][1]+data)
            myglobal.node.dict_qid[str(qid)][0].set()
            return "ok",200
        else:    
            data=request.json["data"]
...
def get_all():
    ...
    def inthread(qid,host):
        ...
        res=requests.post(url,json={"data":"Node (ip:port):"+ myglobal.node.host+'|'+"Data stored :"+ str(list(myglobal.node.dict_db.values()))+"\n"})
