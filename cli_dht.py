...
# IP addresses:ports of 5 VMs in the Ocean
ip_adds = ["192.168.1.1:5000","192.168.1.1:5001","192.168.1.2:5000","192.168.1.2:5001","192.168.1.3:5000","192.168.1.3:5001","192.168.1.4:5000","192.168.1.4:5001","192.168.1.5:5000","192.168.1.5:5001"]
...
def nodes2():
    ...
    hashed_ips = sorted(hashed_ips, key = lambda x: x[0])  # a list of IDs in increasing order according to initial Chord's topology
...
# @main.command(name='initialize', help='Finds active hosts')
# def initialize():
    # set_addresses()
...
# @main.command(name='dht', help='Setting DHT Chord')
# @click.argument('num', default = 1, type=int)
# def dht(num):
#     print()
#     print("Ping clients to join chord")
#     url_str = "http://localhost:500"
#     for i in range(num-1):
#         url = url_str + str(i+1)+"/"
#         res = requests.get(url)
#         if res.status_code == 200:
#             click.echo("Pinged " + url_str + str(i+1) + " successfully!")
#         else:
#             click.echo("Error! Couldn't ping " + url)
#             click.echo('\n')
#         time.sleep(1)
#     print()

@main.command(name='insert', help='Insert key-value pair')
@click.option('--node', '-n', required=False, default = -1, type=int, help='Give num of node to send request according to its position to Chord, eg. press "1" to send to 1st node')
